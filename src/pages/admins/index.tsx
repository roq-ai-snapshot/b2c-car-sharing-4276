...
import { getAdmins, deleteAdminById } from 'apiSdk/admins';
import { AdminInterface } from 'interfaces/admin';

type ColumnType = ColumnDef<AdminInterface, unknown>;
...
export function AdminListPage(props: AdminListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<AdminInterface>>(
    () => `/admins?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: AdminInterface) => {
    if (hasAccess('admin', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/admins/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/admins/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('admin', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/admins/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('admin', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      ...
      >
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="admin" />
        </Flex>

        {hasAccess('admin', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/admins/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
